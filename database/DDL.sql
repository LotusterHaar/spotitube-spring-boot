...
CREATE TABLE IF NOT EXISTS `spotitube`.`track` (
  ...
  INDEX `song_id_idx` (`song_or_video` ASC) VISIBLE)
ENGINE = InnoDB
AUTO_INCREMENT = 21
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_bin;
...
CREATE TABLE IF NOT EXISTS `spotitube`.`songtrack` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  CONSTRAINT `song_or_video1`
    FOREIGN KEY (`id`)
    REFERENCES `spotitube`.`track` (`song_or_video`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 10
...
USE `spotitube`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `spotitube`.`tracks_view` AS select `spotitube`.`track`.`id` AS `id`,`spotitube`.`track`.`title` AS `title`,`spotitube`.`track`.`performer` AS `performer`,`spotitube`.`track`.`duration` AS `duration`,`spotitube`.`track`.`playcount` AS `playcount`,`spotitube`.`track`.`offlineAvailable` AS `offlineAvailable`,`spotitube`.`songtrack`.`album` AS `album`,`spotitube`.`videotrack`.`publicationDate` AS `publicationDate`,`spotitube`.`videotrack`.`description` AS `description` from ((`spotitube`.`track` left join `spotitube`.`songtrack` on((`spotitube`.`songtrack`.`id` = `spotitube`.`track`.`song_or_video`))) left join `spotitube`.`videotrack` on((`spotitube`.`videotrack`.`id` = `spotitube`.`track`.`song_or_video`)));
